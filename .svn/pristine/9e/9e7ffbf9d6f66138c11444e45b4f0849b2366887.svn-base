 ...
 */
void
dbe_print(struct db_entry* entry,FILE* db_file)
{
...
do_export_db(struct db_entry** db, const char* filename)
{
  ...
  // * close the file.
  FILE* write_f = fopen(filename,"w");
  int b;
  for(b=0;b<db_count_entries(db);b++) {
    dbe_print(db[b],write_f);
  }
  close(write_f);
}
...
do_import_db(struct db_entry** db, const char* filename)
{
  ...
  char name[512];
  char value[512];
  FILE* read_f=fopen(filename,"r");
  while(fscanf(read_f,"%s => %s",name,value)!=EOF){
...
getALine(char* dest, int maxlen, FILE* fd)
{
  ...
  // Don't forget to add a null character to the end of
  // whatever you read into dest!
