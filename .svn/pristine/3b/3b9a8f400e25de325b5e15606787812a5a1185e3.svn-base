...
test_do_find_all_matches(void)
{
  ...
  //TODO finish this test HERE:
  // you'll need to call do_find_all_matches in a way that finds something put into
  // the database above, then check to see what showed up in G_PRINTF_OUTPUT.
  // It will look similar to this:
  //do_find_all_matches(db, ...);
  //TEST_ASSERT_EQUAL_STRING_LEN("Bob => B1\n", G_PRINTF_OUTPUT, ...);
  //...

  do_find_all_matches(db,"Bo");
  getALine(buf,512,stdin);
  TEST_ASSERT_EQUAL_STRING_LEN("Bob => B1\n", G_PRINTF_OUTPUT,10);
  TEST_ASSERT_EQUAL_STRING_LEN("Bonnie => D1\n", G_PRINTF_OUTPUT+10,14); 

  // clean up the database
  while(db_count_entries(db) > 0) { db_remove(db, 0); }
  free(db);

  //TODO: remove this comment and the next line when you've finished writing the test.
  //TEST_IGNORE_MESSAGE("Finish this test after you implement find!");
}
...
test_getALine(void)
{
  ...
  const char* S_TEST3 = "this is a line,hahahahahahhahahahahahahahhahahaha";
